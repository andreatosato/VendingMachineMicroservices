...
{
    public class AutenticatedBlazorUserHttpClient : IAuthUserClient
    {
        private readonly IAuthenticationApi authenticationApi;

        public AutenticatedBlazorUserHttpClient(IAuthenticationApi authenticationApi)
        {
        ...
        public HttpClient GetClient(string baseUrl, string token)
        {
            ...
            var authHandler = new AuthenticatedBlazorHttpClientHandler(() => Task.FromResult(token))
            {
